...
template <typename T> inline void malloc_run_state(RunState<T> *s, Config *p) {
  ...
  try {
    s->input = allocateTensor<T>(p->dim);
    ...
    unsigned long longest_state_tensor_len = std::max({
      p->dim, 2 * p->d_inner, p->d_inner * p->d_state, p->rounded_vocab_size,
      p->d_inner * p->d_conv, p->dt_rank + 2 * p->d_state
    });

    //TODO change longest_tensor_len to a property of state
    s->dequantized_buffer =
        static_cast<float *>(malloc(longest_state_tensor_len * sizeof(float)));
...
template <typename T> inline void reset_internal_state(Mamba<T> *mamba) {
  // reset the internal state of the models
...
inline void memory_map_weights(MambaWeights<T> *w, Config *p, T *ptr) {
  ...
  w->token_embedding_table = Tensor<T>(ptr, false);
  ptr += p->rounded_vocab_size * p->dim;

  // TODO need to read scales and zeropoints
  // VERY BIG TODO

  w->in_proj = Tensor2D<T>(ptr, (2 * p->d_inner) * p->dim, false);
  ptr += n_layers * (2 * p->d_inner) * p->dim;

  w->conv1d_weight = Tensor2D<T>(ptr, p->d_inner * 1 * p->d_conv, false);
  ptr += n_layers * p->d_inner * 1 * p->d_conv;

  w->conv1d_bias = Tensor2D<T>(ptr, p->d_inner, false);
  ptr += n_layers * p->d_inner;

  w->x_proj = Tensor2D<T>(ptr, (p->dt_rank + 2 * p->d_state) * p->d_inner, false);
  ptr += n_layers * (p->dt_rank + 2 * p->d_state) * p->d_inner;

  w->dt_proj_weight = Tensor2D<T>(ptr, p->d_inner * p->dt_rank, false);
  ptr += n_layers * p->d_inner * p->dt_rank;

  w->dt_proj_bias = Tensor2D<T>(ptr, p->d_inner, false);
  ptr += n_layers * p->d_inner;

  w->A = Tensor2D<T>(ptr, p->d_inner * p->d_state, false);
  ptr += n_layers * p->d_inner * p->d_state;

  w->D = Tensor2D<T>(ptr, p->d_inner, false);
  ptr += n_layers * p->d_inner;

  w->out_proj = Tensor2D<T>(ptr, p->dim * p->d_inner, false);
  ptr += n_layers * p->dim * p->d_inner;

  w->norm = Tensor2D<T>(ptr, p->dim, false);
  ptr += n_layers * p->dim;

  w->final_norm = Tensor<T>(ptr, false);
  ptr += p->dim;
...
inline Tensor<T> quantized_to_float_tensor(size_t dim, uint8_t num_bits,
  ...
  memcpy(tensor_data, ptr, dim * sizeof(T));
  ptr += dim * sizeof(T);
...
inline void load_model_file(std::string model_path, Config *config,
  ...
  *file_size = file.tellg();
  ...
  // memory map the models weights into the data pointer
  ...
  *data = static_cast<T *>(
  ...
  T *weights_ptr = *data + (256 / sizeof(T));
...
inline void load_model(Mamba<T> *m, std::string model_path) {
  // read the Config and the Weights from the models file
...
inline long time_in_ms() {
  // return time in milliseconds, for benchmarking the models speed
...
inline void apply_repetition_penalty(EnhancedTensor<T> &logits,
                                     ...
                                     const float penalty, float* templogits) {
  if (penalty == 1.0)
    return;
  ...
  for (int & prev_token : prev_tokens) {
    const float score = templogits[prev_token];
    templogits[prev_token] = score > 0 ? score * penalty : score / penalty;
  }
