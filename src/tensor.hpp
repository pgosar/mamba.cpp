...
template <Number T> class TensorBase {};

template <Number T> class Tensor : TensorBase<T> {
protected:
...
public:
  Tensor(const float scale, const float zeropoint, T *data)
      : _scale(scale), _zeropoint(zeropoint), _data(data), _owner(true) {}

  explicit Tensor(T *const data)
      : _scale(1.0f), _zeropoint(0.0f), _data(data), _owner(true) {}

  Tensor(T *const data, const bool owner)
      : _scale(1.0f), _zeropoint(0.0f), _data(data), _owner(owner) {}

  Tensor() : _scale(1.0f), _zeropoint(0.0f), _data(nullptr), _owner(true) {}

  Tensor(const float scale, const float zeropoint, T *data, bool owner)
      : _scale(scale), _zeropoint(zeropoint), _data(data), _owner(owner) {}
  ...
  Tensor(const Tensor<T> &other)
      : _scale(other._scale), _zeropoint(other._zeropoint), _data(other._data)
  ...
  template <typename X = T, std::enable_if_t<std::is_same_v<X, float>>>
  ...
  inline void quantize(size_t i, float value) const {
    _data[i] = static_cast<T>(((value * _scale) + _zeropoint) +
                              .5 * std::signbit(value));
  }
  ...
  [[nodiscard]] const T *data() const { return const_cast<T *>(_data); }

  [[nodiscard]] T *data() { return _data; }

  [[nodiscard]] float scale() const { return _scale; }

  [[nodiscard]] float zeropoint() const { return _zeropoint; }

  [[nodiscard]] T get(size_t i) const { return this->_data[i]; }

  void set(size_t i, T d) { this->_data[i] = d; }

  Tensor<T> operator+(const size_t off) {
    return Tensor<T>(this->_scale, this->_zeropoint, this->_data + off, false);
  }

  void copy_data(Tensor<T> &other, int n) {
 ...
 */
template <Number T> class EnhancedTensor : public Tensor<T> {
private:
  size_t _len;
...
public:
  ...
  EnhancedTensor(float scale, float zeropoint, T *data, const size_t len,
                 bool owner)
      : Tensor<T>(scale, zeropoint, data, owner), _len(len) {}

  EnhancedTensor(T *const data, size_t len) : Tensor<T>(data), _len(len) {}

  EnhancedTensor() : Tensor<T>(), _len(0) {}
  ...
  template <typename X = T>
  typename std::enable_if_t<!std::is_same_v<X, float>, void>
  ...
  typename std::enable_if_t<std::is_same_v<X, float>, void>
  dequantize(float *dequantize_buffer) {
    memcpy(dequantize_buffer, this->_data, _len * sizeof(float));
  }
  ...
  requantize(EnhancedTensor<float> &t) {
    ...
    for (int i = 0; i < _len; i++) {
      ...
      if (converted > T_MAX - 1)
        this->_data[i] = T_MAX - 1;
      else if (converted < -T_MAX)
        this->_data[i] = -T_MAX;
      else
        this->_data[i] =
            static_cast<T>(converted + .5 * std::signbit(converted));
    }
  ...
  typename std::enable_if_t<std::is_same_v<X, float>>
  requantize(EnhancedTensor<float> &t) {
    memcpy(this->_data, t._data, _len * sizeof(T));
    t.detach();
  }
  ...
  requantize(const float *t) {
    ...
    constexpr int T_MAX = (1 << (sizeof(T) * 8 - 1));
  ...
  typename std::enable_if_t<std::is_same_v<X, float>>
  requantize(const float *t) {
    memcpy(this->_data, t, _len * sizeof(T));
  }

  // Used to prevent data from being freed on destruction, if using a shared
  // buffer
  void detach() { this->_data = nullptr; }

  [[nodiscard]] size_t len() const { return _len; }

  EnhancedTensor<T> operator+(const size_t off) {
    return EnhancedTensor<T>(this->_scale, this->_zeropoint, this->_data + off,
                             _len - off, false);
  }
...
template <Number T> class Tensor2D {
protected:
  float *_scales;
  float *_zeropoints;
  size_t _layer_len;
...
public:
  Tensor2D(float *scales, float *zeropoints, const size_t layer_len, T *data)
      : _scales(scales), _zeropoints(zeropoints), _layer_len(layer_len),
        _data(data), _owner(true) {}

  Tensor2D(T *data, const size_t layer_len)
      : _scales(nullptr), _zeropoints(nullptr), _layer_len(layer_len),
        _data(data), _owner(true) {}

  Tensor2D(T *data, const size_t layer_len, bool owner)
      : _scales(nullptr), _zeropoints(nullptr), _layer_len(layer_len),
        _data(data), _owner(owner) {}
  ...
  void quantize(size_t i, const float value) const {
    float scale = _scales[i / _layer_len];
    float zeropoint = _zeropoints[i / _layer_len];
    _data[i] = static_cast<T>(((value * scale) + zeropoint) +
                              .5 * std::signbit(value));
  }
  ...
  [[nodiscard]] const T *data() const { return const_cast<T *>(data); }
  ...
  template <typename X = T>
  typename std::enable_if_t<!std::is_same_v<X, float>, EnhancedTensor<T>>
  layer(const size_t l) {
    return EnhancedTensor<T>(_scales[l], _zeropoints[l], _data + l * _layer_len,
                             _layer_len, false);
  }
  ...
  typename std::enable_if_t<std::is_same_v<X, float>, EnhancedTensor<T>>
  layer(const size_t l) {
    return EnhancedTensor<T>(1.0f, 0, _data + l * _layer_len, _layer_len,
                             false);
  }

  template <typename X = T>
  typename std::enable_if_t<!std::is_same_v<X, float>, void>
  update_layer(const size_t l, const EnhancedTensor<T> &layer) {
...
template <Number T> class EnhancedTensor2D : public Tensor2D<T> {
private:
  size_t _n_layers;

public:
  EnhancedTensor2D() : Tensor2D<T>(), _n_layers(0) {}
  ...
  EnhancedTensor2D(T *data, size_t layer_len, size_t n_layers)
    ...
    this->_zeropoints = static_cast<float *>(calloc(n_layers, sizeof(float)));
  }

  EnhancedTensor2D(const float *scales, const float *zeropoints, T *data,
                   size_t n_layers)
  ...
  void requantize(EnhancedTensor2D<float> &t) {
    for (int l = 0; l < _n_layers; l++) {
      ...
      for (int i = 0; i < this->_layer_len; i++) {
        float converted =
            dequantized_layer[i] * this->_scales[l] + this->_zeropoints[l];
        if (converted > T_MAX - 1)
          layer[i] = T_MAX - 1;
        else if (converted < -T_MAX)
          layer[i] = -T_MAX;
        else
          layer[i] = static_cast<T>(converted + .5 * std::signbit(converted));
      }
    }
  }
