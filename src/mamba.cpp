/* Inference for Mamba models in pure C */
...
void forward_layer(Mamba<T> *mamba, size_t l, EnhancedTensor<T> &hidden_state) {
  ...
  EnhancedTensor<T> z = s->xz.subset(d_inner, d_inner); // z (d_inner)
  ...
  linear(s->dt, s->x_db, w->dt_proj_weight.layer(l), w->dt_proj_bias.layer(l),
         tempbuf, d_inner, dt_rank);
  EnhancedTensor<T> &dt =
      s->dt; // NOTE: dt is now bigger: (d_inner) instead of (dt_rank)
  // dt = F.softplus(dt)
  softplus(dt, tempbuf);
...
template <typename T> EnhancedTensor<T> &forward(Mamba<T> *mamba, int token) {
  ...
  for (int l = 0; l < p->n_layers; l++) {
    ...
    for (int i = 0; i < dim; i++) {
      s->dequantized_buffer[i] = inputTensor[i] + hidden_state[i];
    }

    hidden_state.requantize(s->dequantized_buffer);

    // copy hidden_state back into input for the next layer
    inputTensor.copy_data(hidden_state, dim);
  }

  // s->input.requantize(input_data); TODO wait what
...
void generate(Mamba<T> *mamba, Tokenizer *tokenizer, Sampler *sampler,
              const std::string &prompt, int steps, UserConfig userConfig) {
  // encode the (string) prompt into tokens sequence
  int num_prompt_tokens = 0;
  const auto prompt_tokens = static_cast<int *>(
      malloc((prompt.length() + 3) * sizeof(int))); // +3 for '\0', BOS, EOS
  tokenizer->encode(prompt, 0, 0, prompt_tokens, &num_prompt_tokens);
  ...
  if (num_prompt_tokens > 1) {
    char *piece = tokenizer->decode(Tokenizer::EOS, prompt_tokens[0]);
    safe_printf(piece);
    fflush(stdout);
  }
  ...
  while (pos < steps) {
    // forward the models to get logits for the next token
    ...
    if (pos < num_prompt_tokens - 1) {
      ...
      apply_repetition_penalty(logits, prev_tokens,
                               ...
                               mamba->state.dequantized_buffer);

      next = sampler->sample(logits, mamba->state.dequantized_buffer);
    }
    pos++;
...
void chat(Mamba<T> *mamba, Tokenizer *tokenizer, Sampler *sampler,
  ...
  int next;             // will store the next token in the sequence
  int token;            // stores the current token to feed into the models
  int pos = 0;          // position in the sequence
  while (pos < steps) {
    ...
    if (user_turn) {
      ...
      tokenizer->encode(rendered_prompt, 0, 0, prompt_tokens,
                        &num_prompt_tokens);
    ...
    // determine the token to pass into the models next
    ...
    if (user_idx >= num_prompt_tokens && next != Tokenizer::EOS) {
...
void error_usage() {
  ...
  std::cerr << "Example: run models.bin -n 256 -i \"Once upon a time\"\n";
  std::cerr << "Options:\n";
  ...
  std::cerr
      << "  --specdecoding enable speculative decoding with path to models\n";
  exit(EXIT_FAILURE);
}

template <typename T>
void run_mamba(const std::string &model_path, const std::string &tokenizer_path,
               const std::string &mode, const std::string &prompt,
               const std::string &system_prompt, int steps, float temperature,
               float topp, const UserConfig &user_config) {
  Mamba<T> mamba;
  ...
  Sampler sampler(mamba.config.vocab_size, temperature, topp);

  if (mode == "generate") {
    generate(&mamba, &tokenizer, &sampler, prompt, steps, user_config);
  } else if (mode == "chat") {
...
int main(int argc, char *argv[]) {
  ...
  args::ArgumentParser parser("Run the models with specified options.");
  ...
  args::ValueFlag<std::string> specDecodingFlag(
      parser, "specdecoding", "Path to the models for speculative decoding",
      {"specdecoding"}, "models/model.bin");
  ...
  std::string specDecodingModelPath = args::get(specDecodingFlag);
  ...
  // Handle rounded vocab size
  config.rounded_vocab_size =
      (config.vocab_size % 8 == 0)
          ? config.vocab_size
  ...
  // Dispatch to appropriate template based on quantization
  try {
    switch (config.num_bits) {
    case 32:
      run_mamba<float>(model_path, tokenizer_path, mode, prompt, system_prompt,
                       steps, temperature, topp, userConfig);
      break;
    case 16:
      run_mamba<int16_t>(model_path, tokenizer_path, mode, prompt,
                         system_prompt, steps, temperature, topp, userConfig);
      break;
    case 8:
      run_mamba<int8_t>(model_path, tokenizer_path, mode, prompt, system_prompt,
                        steps, temperature, topp, userConfig);
      break;
    default:
      throw std::runtime_error("Unsupported quantization bits: " +
