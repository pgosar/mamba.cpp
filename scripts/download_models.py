...
def serialize_fp32(
    ...
    if num_bits != 32 and num_bits != 16 and num_bits != 8:
        print(f"Unsupported num bits: {num_bits}")
        exit(1)
    ...
    if num_bits == 16:
        ...
        # need to specify endianess here
        b: bytes = struct.pack(f"<ff{len(d)}b", scale, zeropoint, *d.numpy())
    else:
        b: bytes = struct.pack(
            f"{len(d)}f", *d.numpy()
...
def model_export(
    ...
    with open(path, "wb") as f:
        ...
        for layer in layer_weights:
            for n in range(config.n_layer):
                largest_tensor = max(largest_tensor, model_dict[layer % n].size(0))
...
def main() -> None:
    ...
    _: Action = parser.add_argument(
        ...
        help="models name (allowed options: 130m, 370m, 790m, 1.4b, 2.8b)",
        default="130m",
    )
    _: Action = parser.add_argument(
        "-md",
    ...
    model: MambaForCausalLM = MambaForCausalLM.from_pretrained(
        "state-spaces/mamba-" + args.model + "-hf")  # type: ignore
    ...
    print("models exported to ", args.model)
